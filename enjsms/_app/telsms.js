...
_gsm('set ta._smsIncomeCheckTimeSec: ' + ta._smsIncomeCheckTimeSec)
                        ta.curm.incomeCheckH = setTimeout(function(){
                            ta.qcmds(ta._cmd_smsmemr ,true)
                        }, ta._smsIncomeCheckTimeSec)

                        if(ta.rcvd_sms.length && actnums()){// [activation] actnums ret phony
...
//_gsm('ta._smsle: ' + ta._smsle)
    ...
    ,this.do_at_timeout = function(e){
        ...
        return _err('do clean up by ESC')
        //process.nextTick(_do_TE2ME_cmd_loop)
    }
...
_gsm("append_this_cmds: " + append_this_cmds)
        if ('string' === typeof append_this_cmds) {
            if (append_this_cmds.length > 0) {
                ...
                with (ta.curm.cmdq) {
                    push(ta._cmd_get)
                    if(ta.atsetup) push(ta.atsetup)
                    push(append_this_cmds)
                    push(ta._cmd_release)
                }
            }
        } else {
...
_gsm('gsmtel_addr: ' + inspect(gsmtel_addr))
        ...
        } catch(e) { return _err('FATAL ERROR: parsing GSM_SETUP') }
    } else if(/^\d*/.test(process.env.JSLAVE)){
        try {
            gsmtel_addr = JSON.parse(process.env.GSM_SETUP)
_gsm('gsmtel_addr: ' + inspect(gsmtel_addr))
            gsmtel_addr = gsmtel_addr[process.env.JSLAVE]
        } catch(e) { return _err('FATAL ERROR: parsing GSM_SETUP[JSLAVE]') }
    } else return _err('FATAL ERROR: env.JSLAVE is not number for env.GSM_SETUP')
_gsm('gsmtel_addr: ' + inspect(gsmtel_addr))
...
app.del('/taq.json/:id', function (req, res) {
    var id = mongo.ObjectId(String(req.params.id).split('_')[0])
        ,r = { success: false }

    taq.findOne({ _id: id ,got:{ $exists: !true } } ,function(e ,d){
        if(e || !d) return _err('del taq: ' + e) ,res.json(r)
        return taq.remove({ _id: id ,got:{ $exists: !true } } ,function(e){
            if(e) return _err(e) ,res.json(r)
            r.success = true ,res.json(r)
            return taq.update({ _id: taqf._id } ,{ $inc: { taq_count: -d.parts.length }}
            ,function(e, q){
